...
class TrainingSessionSerializer(serializers.ModelSerializer):
    """Training session for the court object."""
    court = CourtSerializer(read_only=True)
    court_id = serializers.PrimaryKeyRelatedField(source='court', queryset=Court.objects.all(), write_only=True)
    client = UserSerializer(read_only=True)
    client_id = serializers.PrimaryKeyRelatedField(source='client', queryset=CustomUser.objects.all(), write_only=True)
    coach = UserSerializer(read_only=True)
    coach_id = serializers.PrimaryKeyRelatedField(source='coach', queryset=CustomUser.objects.all(), write_only=True)
    ...
    def validate(self, attrs):
        ...
        if TrainingSession.objects.filter(court=attrs.get('court'),
                ...
                raise serializers.ValidationError("Court already taken.")
